...
exports.insertOrderRaw = insertOrderRaw;
const redis_1 = require("../utils/redis");
async function insertOrderRaw(req, res) {
    try {
        ...
        const id = await redis_1.redis.xadd("stream:orders", "*", "symbol", String(symbol), "side", String(side), "qty", String(qty), "userId", String(userId), "amount", String(amount ?? 0), "leverage", String(leverage ?? 0), "event", String(event ?? "insert"), "ts", String(Date.now()));
